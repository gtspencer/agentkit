...
`,
  ...
  async createStream(
    ...
    try {
      ...
      return `Created stream of token ${args.erc20TokenAddress} to ${args.recipientAddress} at a rate of ${args.flowRate}. The link to the stream is ${this.getStreamLink(walletProvider.getNetwork(), args.erc20TokenAddress, walletProvider.getAddress(), args.recipientAddress)}.  Transaction hash: ${streamHash}`;
    } catch (error) {
...
`,
  ...
  async updateStream(
    ...
    try {
      ...
      return `Updated stream of token ${args.erc20TokenAddress} to ${args.recipientAddress} at a rate of ${args.flowRate}.  Transaction hash: ${hash}`;
    } catch (error) {
...
`,
  ...
  async deleteStream(
    ...
    try {
      ...
      return `Stopped stream of token ${args.erc20TokenAddress} to ${args.recipientAddress}.  Transaction hash: ${hash}`;
    } catch (error) {
